...
export default function Menu({ cart, onAddToCart }: MenuProps) {
  ...
  // Derive categories from badges
  const categories = ["All Items", ...Array.from(new Set(menu.map((m) => m.badge)))];

  // Filter by category + search
  const filtered = menu.filter((item) => {
    const matchCat = activeCategory === "All Items" || item.badge === activeCategory;
      ...
      <div className="container" style={{ position: "relative" }}>
        ...
        <div className="reveal" style={{ textAlign: "center", marginBottom: "3rem" }}>
          <span style={{
            ...
            textTransform: "uppercase", color: "var(--gold)", display: "block", marginBottom: "0.6rem",
          }}>
            Our Menu
          </span>
          <h2 style={{
            ...
            Pick Your <em style={{ color: "var(--terracotta)", fontStyle: "italic" }}>Favourite</em>
          </h2>
          <p style={{
            color: "var(--text-muted)", fontSize: "0.95rem",
            lineHeight: 1.7, maxWidth: "480px", margin: "0.8rem auto 0",
          }}>
        ...
        {/* ── Search Bar ── */}
        {!loading && !error && (
          <div className="reveal" style={{ maxWidth: "520px", margin: "0 auto 2rem" }}>
            ...
            {categories.map((cat) => (
              <button
                ...
                style={{
                  ...
                  border: `1.5px solid ${activeCategory === cat ? "var(--green)" : "rgba(27,67,50,0.15)"}`,
                  padding: "0.48rem 1.2rem",
                  borderRadius: "var(--radius-full)",
                  fontSize: "0.85rem", fontWeight: 600,
                  cursor: "pointer", transition: "all 0.2s ease",
                  fontFamily: "inherit",
                  boxShadow: activeCategory === cat ? "0 4px 12px rgba(27,67,50,0.2)" : "none",
                }}
                onMouseEnter={(e) => {
        ...
        {!loading && !error && filtered.length === 0 && (
          <div style={{
            ...
            <button onClick={() => { setSearch(""); setActiveCategory("All Items"); }} style={{
              marginTop: "1rem", background: "var(--green)", color: "white",
              border: "none", padding: "0.6rem 1.4rem", borderRadius: "var(--radius-full)",
        ...
        {!loading && !error && filtered.length > 0 && (
          <>
            {/* Result count */}
            <div style={{
              textAlign: "right", marginBottom: "1.2rem",
...
function FoodCard({ item, inCart, onAdd }: {
  item: MenuItem; inCart: boolean; onAdd: () => void;
}) {
    ...
    >
      ...
      <div style={{ position: "relative", height: "220px", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          background: "linear-gradient(to top, rgba(13,43,31,0.55), transparent)",
        }} />
        {/* Badge */}
        <div style={{
          ...
          color: "var(--gold-light)", fontSize: "0.68rem",
          fontWeight: 700, padding: "0.28rem 0.75rem",
      ...
      <div style={{ padding: "1.2rem 1.4rem 1.4rem" }}>
        <h3 style={{
          ...
          color: "var(--green-dark)", marginBottom: "0.4rem", lineHeight: 1.3,
        }}>
          {item.name}
        </h3>
        <p style={{
